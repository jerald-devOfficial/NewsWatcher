...
const Loginview = () => {
  ...
  const handleRegister = (event) => {
    ...
    fetch("/api/users", {
      ...
      .catch((error) => {
        dispatch({
          type: "MSG_DISPLAY",
          msg: `Registration failure: ${error.message}`,
        });
      });
  };
    ...
    fetch("/api/sessions", {
      ...
      .then((response) => {
        ...
        dispatch({
          ...
          msg: `Signed in as ${response.json.displayName}`,
  ...
  const _renderRegisterModal = () => {
    return (
      <Modal show={showModal} onHide={handleCloseRegModal}>
        ...
        <Modal.Footer>
          <Button bsstyle="danger" bssize="lg" onClick={handleCloseRegModal}>
  ...
  if (session) {
    return <h1 id="h1ExistID">Logged in...</h1>;
  } else {
    return (
      <div>
        ...
        <p>
          Not a NewsWatcher user?{" "}
          <button style={{ cursor: "pointer" }} onClick={handleOpenRegModal}>
            Sign Up
          </button>
        </p>
