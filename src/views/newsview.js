...
const NewsView = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/users/${props.session.userId}`, {
      ...
      .then((response) => {
        ...
        setNewsState({
          isLoading: false,
          newsFilters: response.json.newsFilters,
        });
  ...
  const handleShareStory = (index, event) => {
    ...
    fetch("/api/sharednews", {
      ...
      cache: "default",
      body: JSON.stringify(
        newsState.newsFilters[selectedIdx].newsStories[index]
      ),
    })
      .then((res) =>
        res.json().then((json) => ({ ok: res.ok, status: res.status, json }))
  ...
  if (newsState.isLoading) {
    ...
    return (
      <div>
        ...
        <Card
          bg="light"
          key={newsState.newsFilters[selectedIdx].newsStories.length}
        >
        ...
        <ul>
          {newsState.newsFilters[selectedIdx].newsStories.map(
            (newsStory, idx) => (
                    ...
                    <Card.Body>
                      ...
                      <p className="card-text">
                        <small className="text-muted">
                          {newsStory.source} -{" "}
                          <span>{newsStory.hoursString}</span>
                        </small>
                      </p>
                      <p className="card-text">
                        <small className="text-muted">
                          <button
                            ...
                            onClick={(event) => handleShareStory(idx, event)}
                          >
                            Share
                          </button>
                        </small>
