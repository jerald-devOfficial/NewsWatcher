...
const SharedNewsView = (props) => {
  ...
  useEffect(() => {
    ...
    fetch("/api/sharednews", {
      ...
      .then((res) =>
        res.json().then((json) => ({ ok: res.ok, status: res.status, json }))
      )
      .then((response) => {
  ...
  const handleAddComment = (event) => {
    ...
    fetch(
      `/api/sharednews/${newsState.news[selectedStoryIdx].story.storyID}/Comments`,
      {
        method: "POST",
        headers: new Headers({
          "x-auth": props.session.token,
      ...
      .then((response) => {
        ...
        newNewsArray[selectedStoryIdx].comments.push({
      ...
      <div>
            ...
            <Card bg="light" key={idx}>
              <div className="row g-0">
                ...
                <div className="col-md-8">
                  <Card.Body>
                    <h5 className="card-title">
                      <b>{sharedStory.story.title}</b>
                    </h5>
                    <p className="card-text">
                      {sharedStory.story.contentSnippet}
                    </p>
                    <p className="card-text">
                      <small className="text-muted">
                        {sharedStory.story.source} -{" "}
                        <span>{sharedStory.story.hoursString}</span>
                      </small>
                    </p>
                    <p className="card-text">
                      <small className="text-muted">
                        <button
                          ...
                          onClick={(event) => handleOpenModal(idx, event)}
                        >
                          Comments
                        </button>
                      </small>
        ...
        {newsState.news.length > 0 && (
          <Modal show={showModal} onHide={handleCloseModal}>
            ...
            <Modal.Body>
              <Form onSubmit={handleAddComment}>
                <FormGroup controlId="commentList">
                  <FormLabel>
                    <FontAwesomeIcon icon={faUser} /> Comments
                  </FormLabel>
                  <ul
                    style={{
                      height: "10em",
                  ...
                  >
                    {newsState.news[selectedStoryIdx].comments.map(
                      (comment) => (
                        <li>
                          <div>
                            <p>
                              '{comment.comment}' - {comment.displayName}{" "}
                            </p>
                          </div>
                        </li>
                      )
            ...
            <Modal.Footer>
              <Button bsstyle="danger" bssize="lg" onClick={handleCloseModal}>
