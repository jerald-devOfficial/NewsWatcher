...
const ProfileView = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/users/${props.session.userId}`, {
      ...
      .then((response) => {
        ...
        for (let i = 0; i < response.json.newsFilters.length; i++) {
  ...
  const handleUnRegister = (event) => {
    ...
    fetch(`/api/users/${props.session.userId}`, {
      ...
      .catch((error) => {
        dispatch({
          type: "MSG_DISPLAY",
          msg: `Account delete failed: ${error.message}`,
        });
      });
  };
  ...
  const handleAdd = (event) => {
    ...
    if (user.newsFilters.length === 5) {
      ...
      const len = user.newsFilters.length;
      setUser((prevUser) => {
        let newNewFilters = [...prevUser.newsFilters];
        newNewFilters.push({
          name: "New Filter",
      ...
      <div>
        ...
        <Form>
          <FormGroup controlId="formControlsSelect">
            <FormSelect
              aria-label="News filter selection"
        ...
        <Form>
          ...
          >
            <ButtonToolbar>
              <Button
                disabled={user.newsFilters.length > 4}
                bsstyle="primary"
                bssize="sm"
                onClick={handleAdd}
              >
              ...
              <Button
                disabled={user.newsFilters.length < 2}
                bsstyle="primary"
                bssize="sm"
              ...
              <Button bsstyle="primary" bssize="sm" onClick={handleSave}>
        ...
        <hr />
        <p>
          No longer have a need for NewsWatcher?{" "}
          <button
            id="deleteLink"
            style={{ cursor: "pointer" }}
            onClick={handleOpenModal}
          >
            Delete your NewsWatcher Account
          </button>
        </p>
